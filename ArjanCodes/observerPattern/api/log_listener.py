...
def handle_user_password_forgotten_event(user):
    log(f"User with email address {user.email} has requested a passord reset")

def handle_user_upgrade_plan_event(user):
    log(f"User with email address {user.email} has uprageded their plan")

def setup_log_event_handlers():
    subscribe("user_regitered", handle_user_registered_event)
